...
double coalescence_function(double p_px,double p_py,double p_pz,double n_px,double n_py,double n_pz, double pcoal) {
    double mp = 0.938;
    double mn = 0.939;
    double D_m = mn+mp;
...
int main(){
 ...
 // Begin event loop.
	...
	if (idap == -2212 and deuteroncheck==0){ //if the event is an antiproton
		...
		for (int j = 0; j < pythia.event.size(); ++j){ //loop over the final particles
			...
			if (jdap == -2112){ //if the second particle is an antineutrons
				...
				double D_E = coalescence_function(p_px,p_py,p_pz,n_px,n_py,n_pz,pcoalescence);
  ...
  Dbar.table( outdir + "./antideuterons_spectrum_pythia8.dat");
