...
c        initial energy not fixed but distributed according to 
c        s*K1(\sqrt(S)/T) / (4pi*m_init^2*T * K2(m_init/T))^2
c        K1 and K2 are the modified bessel function of the second type: 
c        www.thphys.uni-heidelberg.de/ ̃plehn
c        note bessel function of the second type are not standard
c        sample_get_boltzman generates event according to this distribution
c        and the PDF function is include in that function
         call sample_get_boltzman(sjac, x(ndim), m1, ebeam(1),s(-nbranch))
         stot = s(-nbranch)
         cm_rap = 0d0
         set_cm_rap=.true.
         pi1(0)=dsqrt(stot)/2
         pi1(3)=sqrt(max(pi1(0)**2-m1**2, 0d0))
         pi2(0)=pi1(0)
         pi2(3)=-sqrt(max(pi2(0)**2-m2**2, 0d0))
c         ebeam(1) = pi1(0)
c         ebeam(2) = pi2(0)
      Subroutine sample_get_boltzman(sjac, R,x m, T, s)
      
      double precision sjac
      double precision R  ! random number
      integer ndim        ! probably useless
      double precision m
      double precision s  ! output number
      double precision T
...
c     R = (y - ymin)/(-ymin) = (-T exp(-x/T) *(T^2 + (x+T)^2))-ymin)/(-ymin)
      sqrts = T
      do i=1,1000
         f = (-T*DEXP(-sqrts/T) *(T**2 + (sqrts+T)**2)-ymin)/(-ymin) - R
         if (f.lt.thres) then
            exit
         endif
...
c      so the S depedence drops
      sjac = sjac * dabs(ymin * DEXP(sqrts/T)) * 2 * K1(sqrts/T)
      sjac = sjac / (4*pi*m**2*T*K2(m/T))
      return 
      end
c
c     Help routine for DM case
c
      ...
      if (x.gt.0) then
         K1 = -pi/2d0*BESSEl_Y1(CMPLX(0., x)) + BESSEL_J1(CMPLX(0., x))* CMPLX(0., pi/2.0)
      else
         K1 = -pi/2d0*BESSEl_Y1(CMPLX(0., x)) + BESSEL_J1(CMPLX(0., x))* CMPLX(0., -3*pi/2.0)
      endif
      return
      end
      ...
      pi = 3.1415926535897932d0
      if (x.gt.0) then
         K2 = -pi/2d0*BESSEl_YN(2,CMPLX(0., x)) + BESSEL_JN(2,CMPLX(0., x))* CMPLX(0., pi/2.0)
      else
         K2 = -pi/2d0*BESSEl_YN(2,CMPLX(0., x)) + BESSEL_JN(2,CMPLX(0., x))* CMPLX(0., -3*pi/2.0)
      endif
      return
      end
