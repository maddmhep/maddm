...
!-----------------------------------------------------------------------------------------------------!
        ...
        character(100) maddm_path, atom_resp_path, Xenon10_efficency_path, Xenon1T_efficency_path
        double precision diff_rate_shell(gridsize), rate_vs_time_shell(day_bins), tot_rate_shell
        double precision dRdS2_shell(gridsize)
        ...
        double precision k_e(gridsize+1), E_e(gridsize+1)
        double precision dayvalue(day_bins+1), daymid(day_bins)
        double precision dRdS2_shell_Xenon10(S2_max), dRdS2_shell_Xenon1T(S2_max)
        double precision dRdS2_Xenon10(S2_max), dRdS2_Xenon1T(S2_max)
        double precision Xenon10_efficency(S2_max), Xenon1T_efficency(S2_max)
        double precision S2_Xenon10(S2_max), S2_Xenon1T(S2_max)
        double precision exposure_Xenon10, exposure_Xenon1T
        parameter(exposure_Xenon10 = 25)        ! Kg days
        parameter(exposure_Xenon1T = 80755)     ! Kg days
        double precision obs_events(7)
...
c       Initialize the parameters and variables
        M_dm = 1 * GeV * cs**(-2) ! mdm(1) * GeV * cs**(-2) ! dark matter mass  REMOVE 
        write(*,*) 'M_dm           :    ' , M_dm, 'GeV'
        ...
        Xenon10_efficency_path = trim(maddm_path) // '/Efficencies/tot_efficency_Xenon10.txt'
        Xenon1T_efficency_path = trim(maddm_path) // '/Efficencies/tot_efficency_Xenon1T.txt'

c       Read the efficencies
        call read_efficency(S2_Xenon10,Xenon10_efficency,Xenon10_efficency_path)
        call read_efficency(S2_Xenon1T,Xenon1T_efficency,Xenon1T_efficency_path)

c       Multiply S2 for the efficency
        do iS2 = 1,S2_max
            S2_Xenon10(iS2) = S2_Xenon10(iS2) * Xenon10_efficency(iS2)
            S2_Xenon1T(iS2) = S2_Xenon1T(iS2) * Xenon1T_efficency(iS2)
        enddo
...
c        dm_response = (3.d0/16.d0)*(16.d0 * 1/sqrt(137.d0) * M_dm * 1.0E-5)**2 ! O4 REMOVE
        dm_response = 16 * pi * ((M_dm+M_e)*GeVtoKg)**2 * 1.0E-36 ! Kg**2 cm**2
...
c       The arrays are of shape gridsize+1 because we need (gridsize+1)-elements to compute (gridsize)-differentials
        do ik = 1,gridsize+1
...
c       Sum over all shells
        do i=1,tot_num_shell
            ...
            call Theory_Simulation(M_dm, M_e, E_e, ioniz_amplitude,
...
c           Avoid to compute the limits for the shells that doesn't contribute to the rate
            if (tot_rate_shell.ne.0) then
                call get_dRdS2_shell(E_e, diff_rate_shell, dRdS2_shell_Xenon10, dRdS2_shell_Xenon1T, target, shell_names(i),i)
            endif
...
c       Multiply for the exposure
        do iS2 = 1,S2_max
            dRdS2_Xenon10(iS2) = dRdS2_Xenon10(iS2) * exposure_Xenon10
            dRdS2_Xenon1T(iS2) = dRdS2_Xenon1T(iS2) * exposure_Xenon1T
        enddo
...
c       Xenon10
        do iS2=1,S2_max
            if(14.le.S2_Xenon10(iS2).and.S2_Xenon10(iS2).lt.41) then
                obs_events(1) = obs_events(1) + dRdS2_Xenon10(iS2)*(S2_Xenon10(iS2+1)-S2_Xenon10(iS2))
            else if(41.le.S2_Xenon10(iS2).and.S2_Xenon10(iS2).lt.68) then
                obs_events(2) = obs_events(2) + dRdS2_Xenon10(iS2)*(S2_Xenon10(iS2+1)-S2_Xenon10(iS2))
            else if(68.le.S2_Xenon10(iS2).and.S2_Xenon10(iS2).lt.95) then
                obs_events(3) = obs_events(3) + dRdS2_Xenon10(iS2)*(S2_Xenon10(iS2+1)-S2_Xenon10(iS2))
            else if(95.le.S2_Xenon10(iS2).and.S2_Xenon10(iS2).lt.122) then
                obs_events(4) = obs_events(4) + dRdS2_Xenon10(iS2)*(S2_Xenon10(iS2+1)-S2_Xenon10(iS2))
            else if(122.le.S2_Xenon10(iS2).and.S2_Xenon10(iS2).lt.149) then
                obs_events(5) = obs_events(5) + dRdS2_Xenon10(iS2)*(S2_Xenon10(iS2+1)-S2_Xenon10(iS2))
            else if(149.le.S2_Xenon10(iS2).and.S2_Xenon10(iS2).lt.176) then
                obs_events(6) = obs_events(6) + dRdS2_Xenon10(iS2)*(S2_Xenon10(iS2+1)-S2_Xenon10(iS2))
            else if(176.le.S2_Xenon10(iS2).and.S2_Xenon10(iS2).lt.203) then
                obs_events(7) = obs_events(7) + dRdS2_Xenon10(iS2)*(S2_Xenon10(iS2+1)-S2_Xenon10(iS2))
            endif
        enddo
        ...
        write(*,*) '|                Xenon10                 |'
        write(*,*) '------------------------------------------'
        write(*,*) '| bin[S2]    |   obs. events             |'
        write(*,*) '|----------------------------------------|'
        write(*,*) '| [14,41)    |' , obs_events(1) , '|'
        write(*,*) '| [41,68)    |' , obs_events(2) , '|'
        write(*,*) '| [68,95)    |' , obs_events(3) , '|'
        write(*,*) '| [95,122)   |' , obs_events(4) , '|'
        write(*,*) '| [122,149)  |' , obs_events(5) , '|'
        write(*,*) '| [149,176)  |' , obs_events(6) , '|'
        write(*,*) '| [176,203)  |' , obs_events(7) , '|'
        write(*,*) '------------------------------------------'
...
c       Xenon1T
        do iS2=1,S2_max
            if(150.le.S2_Xenon1T(iS2).and.S2_Xenon1T(iS2).lt.200) then
                obs_events(1) = obs_events(1) + dRdS2_Xenon1T(iS2)*(S2_Xenon1T(iS2+1)-S2_Xenon1T(iS2))
            else if(200.le.S2_Xenon1T(iS2).and.S2_Xenon1T(iS2).lt.250) then
                obs_events(2) = obs_events(2) + dRdS2_Xenon1T(iS2)*(S2_Xenon1T(iS2+1)-S2_Xenon1T(iS2))
            else if(250.le.S2_Xenon1T(iS2).and.S2_Xenon1T(iS2).lt.300) then
                obs_events(3) = obs_events(3) + dRdS2_Xenon1T(iS2)*(S2_Xenon1T(iS2+1)-S2_Xenon1T(iS2))
            else if(300.le.S2_Xenon1T(iS2).and.S2_Xenon1T(iS2).lt.350) then
                obs_events(4) = obs_events(4) + dRdS2_Xenon1T(iS2)*(S2_Xenon1T(iS2+1)-S2_Xenon1T(iS2))
            endif
        enddo
        ...
        write(*,*) '| bin[S2]    |   obs. events             |'
        write(*,*) '|----------------------------------------|'
        write(*,*) '| [150,200)  |' , obs_events(1) , '|'
        write(*,*) '| [200,250)  |' , obs_events(2) , '|'
        write(*,*) '| [250,300)  |' , obs_events(3) , '|'
        write(*,*) '| [300,350)  |' , obs_events(4) , '|'
        write(*,*) '------------------------------------------'
...
c       ================================================================
        ...
        do iS2 = 1, S2_max
            write(6,*) S2_Xenon10(iS2), dRdS2_Xenon10(iS2)
        enddo
...
c       ================================================================
        ...
        do iS2 = 1, S2_max
            write(7,*) S2_Xenon1T(iS2), dRdS2_Xenon1T(iS2)
        enddo
...
!-------------------------------------------------------------------------------------------------------!
        subroutine Theory_Simulation(M_dm,M_e,E_e,ioniz_amplitude,E_binding,diff_rate,rate_vs_time,tot_rate)
!-------------------------------------------------------------------------------------------------------!
        implicit none
        include '../include/maddm.inc'
...
!-----------------------------------------------------------------------------------------------!
!	    Calculate the double differential spectrum for dark Matter Directional detection.       !
...
!-----------------------------------------------------------------------------------------------!
        ...
        double precision q_exc(gridsize+1), dq(gridsize)
...
!-----------------------------------------------------------------------------------------------!
        ...
        double precision E_e(gridsize+1), diff_rate_shell(gridsize)
        double precision dRdS2_shell_Xenon10(S2_max), dRdS2_shell_Xenon1T(S2_max)
        ...
        integer n_1(gridsize), n_2(gridsize), n_e(gridsize), n_electr
...
c       Fraction of quanta observed as electrons
        f_e = 0.83
...
c       Compute the differential rate (dRdne) wrt the number of initial scattered electrons (n_electr).
        do n_electr = 1, n_electr_max
            do iE=1,gridsize
c               dR/dn_e = integral(dE * P(n_e|E_e) * dR/dE) = integral(dE * P(n_e|E_e) * dR/dlogE * 1/E)
c               Where P(n_e|E_e) is given by a binomial distribution with number of events n_e, number of
c               trials is n_1(E_e)+n_2(E_e) and probability of success is f_e
                dRdne_shell(n_electr) = dRdne_shell(n_electr) +
     &                                  (E_e(iE+1)-E_e(iE)) * diff_rate_shell(iE) / E_e(iE) *
     &                                  binomial_prob(n_electr, n_1(iE)+n_2(iE), f_e)
            enddo
...
c               with mean n_e*g2
     ...
     &                                  gauss_prob(S2, n_electr*g2_Xenon10, sigma_Xenon10)


                dRdS2_shell_Xenon1T(S2) = dRdS2_shell_Xenon1T(S2) +
     &                                  dRdne_shell(n_electr) *
     &                                  gauss_prob(S2, n_electr*g2_Xenon1T, sigma_Xenon1T)
            enddo
...
!---------------------------------------------------------------------------------------------!
        subroutine read_efficency(S2,efficency,path)
!---------------------------------------------------------------------------------------------!
                ...
                double precision S2(S2_max), efficency(S2_max)
                character(100) path
                integer iS2,error

                open(99, FILE = trim(path), action="read")
                
c               Skip the firts 4 rows then read 
                Read(99,*)
                Read(99,*)
                Read(99,*)
                Read(99,*)
                do iS2 =1,S2_max
                    Read(99, *, iostat = error) S2(iS2) , efficency(iS2)
            
                    Select Case(error)
                    Case(0)
c                        write(*,*) iS2 , S2(iS2) , efficency(iS2)
                    Case(iostat_end)
                        Exit
                    Case Default
                        write(*, *) 'Error in reading file'
