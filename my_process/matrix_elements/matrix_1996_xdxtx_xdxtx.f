      SUBROUTINE XDXTX_XDXTX_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/XDXTX_XDXTX_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL XDXTX_XDXTX_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE XDXTX_XDXTX_SMATRIX(P,ANS)
C     
...
C     
C     Process: xd~ t~ > xd~ t~ DMS<=2 SDEFFF=0 SIEFFF=0 @1996
C     
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     put in common block to expose this variable to python interface
      COMMON/XDXTX_XDXTX_PROCESS_NHEL/NHEL
      REAL*8 T
      REAL*8 XDXTX_XDXTX_MATRIX
...
C     
      ...
      DATA USERHEL/-1/
      LOGICAL HELRESET
      COMMON/XDXTX_XDXTX_HELRESET/HELRESET
      DATA HELRESET/.TRUE./
      ...
      COMMON/XDXTX_XDXTX_BORN_BEAM_POL/POLARIZATIONS
      DATA ((POLARIZATIONS(I,J),I=0,NEXTERNAL),J=0,5)/NPOLENTRIES*-1/

C     
C     FUNCTIONS
C     
      LOGICAL XDXTX_XDXTX_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.XDXTX_XDXTX_IS_BORN_HEL_SELECTED(IHEL))) THEN
              CYCLE
            ENDIF
            T=XDXTX_XDXTX_MATRIX(P ,NHEL(1,IHEL),JC(1))
            IF(POLARIZATIONS(0,0).EQ.
     $       -1.OR.XDXTX_XDXTX_IS_BORN_HEL_SELECTED(IHEL)) THEN
              ANS=ANS+T
            ENDIF
      ...
      REAL*8 FUNCTION XDXTX_XDXTX_MATRIX(P,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.9.9, 2022-02-25
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 1
      CALL FFS1_2_0(W(1,4),W(1,2),W(1,5),GC_114,GC_115,AMP(1))
      JAMP(1)=-AMP(1)

      XDXTX_XDXTX_MATRIX = 0.D0
      DO I = 1, NCOLOR
        ZTEMP = (0.D0,0.D0)
        DO J = 1, NCOLOR
          ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
        ENDDO
        XDXTX_XDXTX_MATRIX = XDXTX_XDXTX_MATRIX+ZTEMP*DCONJG(JAMP(I))
      ENDDO

      END

      SUBROUTINE XDXTX_XDXTX_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      IF (NHEL.NE.0) THEN
        CALL XDXTX_XDXTX_SMATRIXHEL(P, NHEL, ANS)
      ELSE
        CALL XDXTX_XDXTX_SMATRIX(P, ANS)
      ENDIF
      RETURN
      ...
      SUBROUTINE XDXTX_XDXTX_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION XDXTX_XDXTX_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      COMMON/XDXTX_XDXTX_PROCESS_NHEL/HELC
...
C     ----------
      ...
      XDXTX_XDXTX_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          XDXTX_XDXTX_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
