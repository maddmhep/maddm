      SUBROUTINE EFT_SI_XDXTX_XDXTX_SMATRIX(P,ANS_SUMMED)
C     
...
C     BEGIN CODE
C     
      CALL EFT_SI_XDXTX_XDXTX_SMATRIX_SPLITORDERS(P,ANS)
      ANS_SUMMED=ANS(0)

      END

      SUBROUTINE EFT_SI_XDXTX_XDXTX_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/EFT_SI_XDXTX_XDXTX_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL EFT_SI_XDXTX_XDXTX_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE EFT_SI_XDXTX_XDXTX_SMATRIX_SPLITORDERS(P,ANS)
C     
...
C     
C     Process: xd~ t~ > xd~ t~ DMS<=2 SIEFFF==2 @1996
C     
      IMPLICIT NONE
C     
C     CONSTANTS
C     
      ...
      COMMON/EFT_SI_XDXTX_XDXTX_CHOSEN_BORN_SQSO/CHOSEN_SO_CONFIGS
C     
C     ARGUMENTS 
C     
...
C     
      ...
      COMMON/EFT_SI_XDXTX_XDXTX_PROCESS_NHEL/NHEL
      ...
      COMMON/EFT_SI_XDXTX_XDXTX_BORN_BEAM_POL/POLARIZATIONS
      DATA ((POLARIZATIONS(I,J),I=0,NEXTERNAL),J=0,5)/NPOLENTRIES*-1/

C     
C     FUNCTIONS
C     
      LOGICAL EFT_SI_XDXTX_XDXTX_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.EFT_SI_XDXTX_XDXTX_IS_BORN_HEL_SELECTED(IHEL)
     $       )) THEN
              CYCLE
            ENDIF
            CALL EFT_SI_XDXTX_XDXTX_MATRIX(P ,NHEL(1,IHEL),JC(1), T)
            BUFF=0D0
            DO I=1,NSQAMPSO
              IF(POLARIZATIONS(0,0).EQ.
     $         -1.OR.EFT_SI_XDXTX_XDXTX_IS_BORN_HEL_SELECTED(IHEL))
     $          THEN
                ANS(I)=ANS(I)+T(I)
              ENDIF
              BUFF=BUFF+T(I)
      ...
      SUBROUTINE EFT_SI_XDXTX_XDXTX_SMATRIXHEL_SPLITORDERS(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     ----------
      ...
      SUBROUTINE EFT_SI_XDXTX_XDXTX_MATRIX(P,NHEL,IC,RES)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.9.9, 2022-02-25
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     FUNCTION
C     
      INTEGER EFT_SI_XDXTX_XDXTX_SQSOINDEX
C     
C     GLOBAL VARIABLES
C     
      INCLUDE 'coupl.inc'
...
C     Amplitude(s) for diagram number 1
      CALL FFFF1_0(W(1,3),W(1,1),W(1,4),W(1,2),GC_3__1,AMP(1))
C     JAMPs contributing to orders SIEFFF=2
      JAMP(1,1)=-AMP(1)
      ...
      DO M = 1, NAMPSO
        DO I = 1, NCOLOR
          ...
          ENDDO
          DO N = 1, NAMPSO
            RES(EFT_SI_XDXTX_XDXTX_SQSOINDEX(M,N)) =
     $        RES(EFT_SI_XDXTX_XDXTX_SQSOINDEX(M,N)) + ZTEMP
     $       *DCONJG(JAMP(I,N))
          ENDDO
        ENDDO
      ENDDO
      ...
      SUBROUTINE EFT_SI_XDXTX_XDXTX_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      IF (NHEL.NE.0) THEN
        CALL EFT_SI_XDXTX_XDXTX_SMATRIXHEL(P, NHEL, ANS)
      ELSE
        CALL EFT_SI_XDXTX_XDXTX_SMATRIX(P, ANS)
      ENDIF
      RETURN
      ...
      SUBROUTINE EFT_SI_XDXTX_XDXTX_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION EFT_SI_XDXTX_XDXTX_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      COMMON/EFT_SI_XDXTX_XDXTX_PROCESS_NHEL/HELC
...
C     ----------
      ...
      EFT_SI_XDXTX_XDXTX_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          EFT_SI_XDXTX_XDXTX_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
...
C     Set of functions to handle the array indices of the split orders
      ...
      INTEGER FUNCTION EFT_SI_XDXTX_XDXTX_SQSOINDEX(ORDERINDEXA,
     $  ORDERINDEXB)
C     
...
C     
      ...
      COMMON/EFT_SI_XDXTX_XDXTX_AMPSPLITORDERS/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER EFT_SI_XDXTX_XDXTX_SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $   +AMPSPLITORDERS(ORDERINDEXB,I)
      ENDDO
      EFT_SI_XDXTX_XDXTX_SQSOINDEX=EFT_SI_XDXTX_XDXTX_SOINDEX_FOR_SQUAR
     $ED_ORDERS(SQORDERS)
      END

      INTEGER FUNCTION
     $  EFT_SI_XDXTX_XDXTX_SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      ...
      COMMON/EFT_SI_XDXTX_XDXTX_SQPLITORDERS/SQPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        EFT_SI_XDXTX_XDXTX_SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping in function'
      WRITE(*,*) 'EFT_SI_xdxtx_xdxtx_SOINDEX_FOR_SQUARED_ORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP

      END

      SUBROUTINE EFT_SI_XDXTX_XDXTX_GET_NSQSO_BORN(NSQSO)
C     
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE EFT_SI_XDXTX_XDXTX_GET_SQUARED_ORDERS_FOR_SOINDEX(SOIN
     $DEX,ORDERS)
C     
...
C     
     ...
     $ //' EFT_SI_xdxtx_xdxtx_GET_SQUARED_ORDERS_FOR_SOINDEX'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE EFT_SI_XDXTX_XDXTX_GET_ORDERS_FOR_AMPSOINDEX(SOINDEX
     $ ,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' EFT_SI_xdxtx_xdxtx_GET_ORDERS_FOR_AMPSOINDEX'
...
C      completeness
      INTEGER FUNCTION EFT_SI_XDXTX_XDXTX_SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        EFT_SI_XDXTX_XDXTX_SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' EFT_SI_xdxtx_xdxtx_SOINDEX_FOR_AMPORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
