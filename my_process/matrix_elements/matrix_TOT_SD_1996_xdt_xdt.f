      SUBROUTINE TOT_SD_XDT_XDT_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/TOT_SD_XDT_XDT_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL TOT_SD_XDT_XDT_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE TOT_SD_XDT_XDT_SMATRIX(P,ANS)
C     
...
C     put in common block to expose this variable to python interface
      COMMON/TOT_SD_XDT_XDT_PROCESS_NHEL/NHEL
      REAL*8 T
      REAL*8 TOT_SD_XDT_XDT_MATRIX
...
C     
      ...
      DATA USERHEL/-1/
      LOGICAL HELRESET
      COMMON/TOT_SD_XDT_XDT_HELRESET/HELRESET
      DATA HELRESET/.TRUE./
      ...
      COMMON/TOT_SD_XDT_XDT_BORN_BEAM_POL/POLARIZATIONS
      DATA ((POLARIZATIONS(I,J),I=0,NEXTERNAL),J=0,5)/NPOLENTRIES*-1/

C     
C     FUNCTIONS
C     
      LOGICAL TOT_SD_XDT_XDT_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.TOT_SD_XDT_XDT_IS_BORN_HEL_SELECTED(IHEL)))
     $        THEN
              CYCLE
            ENDIF
            T=TOT_SD_XDT_XDT_MATRIX(P ,NHEL(1,IHEL),JC(1))
            IF(POLARIZATIONS(0,0).EQ.
     $       -1.OR.TOT_SD_XDT_XDT_IS_BORN_HEL_SELECTED(IHEL)) THEN
              ANS=ANS+T
            ENDIF
      ...
      REAL*8 FUNCTION TOT_SD_XDT_XDT_MATRIX(P,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.9.9, 2022-02-25
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 2
      ...
      TOT_SD_XDT_XDT_MATRIX = 0.D0
      DO I = 1, NCOLOR
        ZTEMP = (0.D0,0.D0)
        DO J = 1, NCOLOR
          ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
        ENDDO
        TOT_SD_XDT_XDT_MATRIX = TOT_SD_XDT_XDT_MATRIX+ZTEMP
     $   *DCONJG(JAMP(I))
      ENDDO

      END

      SUBROUTINE TOT_SD_XDT_XDT_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      IF (NHEL.NE.0) THEN
        CALL TOT_SD_XDT_XDT_SMATRIXHEL(P, NHEL, ANS)
      ELSE
        CALL TOT_SD_XDT_XDT_SMATRIX(P, ANS)
      ENDIF
      RETURN
      ...
      SUBROUTINE TOT_SD_XDT_XDT_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION TOT_SD_XDT_XDT_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      COMMON/TOT_SD_XDT_XDT_PROCESS_NHEL/HELC
...
C     ----------
      ...
      TOT_SD_XDT_XDT_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          TOT_SD_XDT_XDT_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
