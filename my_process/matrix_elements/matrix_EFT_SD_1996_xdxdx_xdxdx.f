      SUBROUTINE EFT_SD_XDXDX_XDXDX_SMATRIX(P,ANS_SUMMED)
C     
...
C     BEGIN CODE
C     
      CALL EFT_SD_XDXDX_XDXDX_SMATRIX_SPLITORDERS(P,ANS)
      ANS_SUMMED=ANS(0)

      END

      SUBROUTINE EFT_SD_XDXDX_XDXDX_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/EFT_SD_XDXDX_XDXDX_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL EFT_SD_XDXDX_XDXDX_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE EFT_SD_XDXDX_XDXDX_SMATRIX_SPLITORDERS(P,ANS)
C     
...
C     
C     Process: xd~ d~ > xd~ d~ DMS<=2 SDEFFF==2 @1996
C     
      IMPLICIT NONE
C     
C     CONSTANTS
C     
      ...
      COMMON/EFT_SD_XDXDX_XDXDX_CHOSEN_BORN_SQSO/CHOSEN_SO_CONFIGS
C     
C     ARGUMENTS 
C     
...
C     
      ...
      COMMON/EFT_SD_XDXDX_XDXDX_PROCESS_NHEL/NHEL
      ...
      COMMON/EFT_SD_XDXDX_XDXDX_BORN_BEAM_POL/POLARIZATIONS
      DATA ((POLARIZATIONS(I,J),I=0,NEXTERNAL),J=0,5)/NPOLENTRIES*-1/

C     
C     FUNCTIONS
C     
      LOGICAL EFT_SD_XDXDX_XDXDX_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.EFT_SD_XDXDX_XDXDX_IS_BORN_HEL_SELECTED(IHEL)
     $       )) THEN
              CYCLE
            ENDIF
            CALL EFT_SD_XDXDX_XDXDX_MATRIX(P ,NHEL(1,IHEL),JC(1), T)
            BUFF=0D0
            DO I=1,NSQAMPSO
              IF(POLARIZATIONS(0,0).EQ.
     $         -1.OR.EFT_SD_XDXDX_XDXDX_IS_BORN_HEL_SELECTED(IHEL))
     $          THEN
                ANS(I)=ANS(I)+T(I)
              ENDIF
              BUFF=BUFF+T(I)
      ...
      SUBROUTINE EFT_SD_XDXDX_XDXDX_SMATRIXHEL_SPLITORDERS(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     ----------
      ...
      SUBROUTINE EFT_SD_XDXDX_XDXDX_MATRIX(P,NHEL,IC,RES)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.9.9, 2022-02-25
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     FUNCTION
C     
      INTEGER EFT_SD_XDXDX_XDXDX_SQSOINDEX
C     
C     GLOBAL VARIABLES
C     
      INCLUDE 'coupl.inc'
...
C     JAMPs contributing to orders SDEFFF=2
      ...
      DO M = 1, NAMPSO
        DO I = 1, NCOLOR
          ...
          ENDDO
          DO N = 1, NAMPSO
            RES(EFT_SD_XDXDX_XDXDX_SQSOINDEX(M,N)) =
     $        RES(EFT_SD_XDXDX_XDXDX_SQSOINDEX(M,N)) + ZTEMP
     $       *DCONJG(JAMP(I,N))
          ENDDO
        ENDDO
      ENDDO
      ...
      SUBROUTINE EFT_SD_XDXDX_XDXDX_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      IF (NHEL.NE.0) THEN
        CALL EFT_SD_XDXDX_XDXDX_SMATRIXHEL(P, NHEL, ANS)
      ELSE
        CALL EFT_SD_XDXDX_XDXDX_SMATRIX(P, ANS)
      ENDIF
      RETURN
      ...
      SUBROUTINE EFT_SD_XDXDX_XDXDX_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION EFT_SD_XDXDX_XDXDX_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      COMMON/EFT_SD_XDXDX_XDXDX_PROCESS_NHEL/HELC
...
C     ----------
      ...
      EFT_SD_XDXDX_XDXDX_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          EFT_SD_XDXDX_XDXDX_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
...
C     Set of functions to handle the array indices of the split orders
      ...
      INTEGER FUNCTION EFT_SD_XDXDX_XDXDX_SQSOINDEX(ORDERINDEXA,
     $  ORDERINDEXB)
C     
...
C     
      ...
      COMMON/EFT_SD_XDXDX_XDXDX_AMPSPLITORDERS/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER EFT_SD_XDXDX_XDXDX_SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $   +AMPSPLITORDERS(ORDERINDEXB,I)
      ENDDO
      EFT_SD_XDXDX_XDXDX_SQSOINDEX=EFT_SD_XDXDX_XDXDX_SOINDEX_FOR_SQUAR
     $ED_ORDERS(SQORDERS)
      END

      INTEGER FUNCTION
     $  EFT_SD_XDXDX_XDXDX_SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      ...
      COMMON/EFT_SD_XDXDX_XDXDX_SQPLITORDERS/SQPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        EFT_SD_XDXDX_XDXDX_SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping in function'
      WRITE(*,*) 'EFT_SD_xdxdx_xdxdx_SOINDEX_FOR_SQUARED_ORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP

      END

      SUBROUTINE EFT_SD_XDXDX_XDXDX_GET_NSQSO_BORN(NSQSO)
C     
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE EFT_SD_XDXDX_XDXDX_GET_SQUARED_ORDERS_FOR_SOINDEX(SOIN
     $DEX,ORDERS)
C     
...
C     
     ...
     $ //' EFT_SD_xdxdx_xdxdx_GET_SQUARED_ORDERS_FOR_SOINDEX'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE EFT_SD_XDXDX_XDXDX_GET_ORDERS_FOR_AMPSOINDEX(SOINDEX
     $ ,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' EFT_SD_xdxdx_xdxdx_GET_ORDERS_FOR_AMPSOINDEX'
...
C      completeness
      INTEGER FUNCTION EFT_SD_XDXDX_XDXDX_SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        EFT_SD_XDXDX_XDXDX_SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' EFT_SD_xdxdx_xdxdx_SOINDEX_FOR_AMPORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
