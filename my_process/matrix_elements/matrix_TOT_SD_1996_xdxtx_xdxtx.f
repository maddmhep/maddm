      SUBROUTINE TOT_SD_XDXTX_XDXTX_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/TOT_SD_XDXTX_XDXTX_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL TOT_SD_XDXTX_XDXTX_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE TOT_SD_XDXTX_XDXTX_SMATRIX(P,ANS)
C     
...
C     
C     Process: xd~ t~ > xd~ t~ DMS<=2 SDEFFF<=99 SIEFFF=0 @1996
C     
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     put in common block to expose this variable to python interface
      COMMON/TOT_SD_XDXTX_XDXTX_PROCESS_NHEL/NHEL
      REAL*8 T
      REAL*8 TOT_SD_XDXTX_XDXTX_MATRIX
...
C     
      ...
      DATA USERHEL/-1/
      LOGICAL HELRESET
      COMMON/TOT_SD_XDXTX_XDXTX_HELRESET/HELRESET
      DATA HELRESET/.TRUE./
      ...
      COMMON/TOT_SD_XDXTX_XDXTX_BORN_BEAM_POL/POLARIZATIONS
      DATA ((POLARIZATIONS(I,J),I=0,NEXTERNAL),J=0,5)/NPOLENTRIES*-1/

C     
C     FUNCTIONS
C     
      LOGICAL TOT_SD_XDXTX_XDXTX_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.TOT_SD_XDXTX_XDXTX_IS_BORN_HEL_SELECTED(IHEL)
     $       )) THEN
              CYCLE
            ENDIF
            T=TOT_SD_XDXTX_XDXTX_MATRIX(P ,NHEL(1,IHEL),JC(1))
            IF(POLARIZATIONS(0,0).EQ.
     $       -1.OR.TOT_SD_XDXTX_XDXTX_IS_BORN_HEL_SELECTED(IHEL)) THEN
              ANS=ANS+T
            ENDIF
      ...
      REAL*8 FUNCTION TOT_SD_XDXTX_XDXTX_MATRIX(P,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.9.9, 2022-02-25
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 1
     ...
     $ ,AMP(1))
      CALL FFS1_2_3(W(1,3),W(1,1),GC_13,GC_14,MDL_MY0,MDL_WY0,W(1,5))
C     Amplitude(s) for diagram number 2
      CALL FFS1_2_0(W(1,4),W(1,2),W(1,5),GC_114,GC_115,AMP(2))
      JAMP(1)=-AMP(1)-AMP(2)

      TOT_SD_XDXTX_XDXTX_MATRIX = 0.D0
      DO I = 1, NCOLOR
        ZTEMP = (0.D0,0.D0)
        DO J = 1, NCOLOR
          ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
        ENDDO
        TOT_SD_XDXTX_XDXTX_MATRIX = TOT_SD_XDXTX_XDXTX_MATRIX+ZTEMP
     $   *DCONJG(JAMP(I))
      ENDDO

      END

      SUBROUTINE TOT_SD_XDXTX_XDXTX_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      IF (NHEL.NE.0) THEN
        CALL TOT_SD_XDXTX_XDXTX_SMATRIXHEL(P, NHEL, ANS)
      ELSE
        CALL TOT_SD_XDXTX_XDXTX_SMATRIX(P, ANS)
      ENDIF
      RETURN
      ...
      SUBROUTINE TOT_SD_XDXTX_XDXTX_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION TOT_SD_XDXTX_XDXTX_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      COMMON/TOT_SD_XDXTX_XDXTX_PROCESS_NHEL/HELC
...
C     ----------
      ...
      TOT_SD_XDXTX_XDXTX_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          TOT_SD_XDXTX_XDXTX_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
